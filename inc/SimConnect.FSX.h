...
typedef DWORD SIMCONNECT_WAYPOINT_FLAGS;
    ...
    static const DWORD SIMCONNECT_WAYPOINT_SPEED_REQUESTED         = 0x04;    // requested speed_restr at waypoint is valid
    static const DWORD SIMCONNECT_WAYPOINT_THROTTLE_REQUESTED      = 0x08;    // request a specific throttle percentage
    static const DWORD SIMCONNECT_WAYPOINT_COMPUTE_VERTICAL_SPEED  = 0x10;    // compute vertical to speed_restr to reach waypoint altitude_restr when crossing the waypoint
...
struct SIMCONNECT_RECV_SIMOBJECT_DATA : public SIMCONNECT_RECV           // when dwID == SIMCONNECT_RECV_ID_SIMOBJECT_DATA
{
    ...
    DWORD   dwentrynumber;      // if multiple SystemObjects returned, this is number <entrynumber> out of <outof>.
